...
function zip(directory, name) {
  ...
  output.on('open', () => {
    ...
    this.serverless.cli.log(
      `${new Date().toUTCString()} Zip ${artifactFilePath} open - ${files.length} files`
    )
    zip.pipe(output);

    _.forEach(files, filePath => {
      const startFile = _.now()
      const fullPath = path.resolve(directory, filePath);

      const stats = fs.statSync(fullPath);

      if (!stats.isDirectory(fullPath)) {
        const readStream = fs.readFileSync(fullPath)
        readStream.on('close', () => {
          const duration = _.now() - startFile
          if (duration >= 1) {
            fileDurations[fullPath] = duration
          }
        })
    ...
    if (this.options.verbose) {
      this.serverless.cli.log(
        `${new Date().toUTCString()} Zip ${directory} finalized`
      )
    }
  });
  ...
  return new BbPromise((resolve, reject) => {
    output.on('close', () => {
      if (this.options.verbose) {
        this.serverless.cli.log(
          `${new Date().toUTCString()} Zip ${artifactFilePath} closed`
        )
        this.serverless.cli.log(JSON.stringify(fileDurations))
      }
...
module.exports = {
  packageModules() {
    ...
    return BbPromise.mapSeries(stats.stats, (compileStats, index) => {
      ...
      this.options.verbose &&
      this.serverless.cli.log(
        `${new Date().toUTCString()} Zip ${_.isEmpty(entryFunction) ? 'service' : 'function'}: ${modulePath} start`
      )
      ...
      return zip
        ...
        .tap(
          () =>
            this.options.verbose &&
            this.serverless.cli.log(
              `${new Date().toUTCString()} Zip ${_.isEmpty(entryFunction) ? 'service' : 'function'}: ${modulePath} [${_.now() - startZip} ms]`
            )
        )
        .then(artifactPath => {
