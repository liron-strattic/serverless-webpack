...
function zip(directory, name) {
  ...
  this.options.verbose &&
    this.serverless.cli.log(`${new Date().toUTCString()} Zip ${directory} into ${artifactFilePath} - start`);
  const startTime = _.now();

  const directoryPath = path.parse(directory);
  const cwd = directoryPath.dir;
  const zipArgs = {
    source: directoryPath.name,
    cwd,
    destination: path.relative(cwd, artifactFilePath)
  };
  this.options.verbose && this.serverless.cli.log(`Zip Args: ${JSON.stringify(zipArgs)}`);
  return new BbPromise((resolve, reject) => {
    bestzip(zipArgs)
      .then(() => {
        const duration = _.now() - startTime;
        this.options.verbose &&
          this.serverless.cli.log(
            `${new Date().toUTCString()} Zip ${directory} into ${artifactFilePath} - finished [${duration} ms]`
          );
